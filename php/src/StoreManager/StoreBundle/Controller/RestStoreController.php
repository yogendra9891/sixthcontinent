...
class RestStoreController extends Controller {
    ...
    protected $cover_crop_image_width = 910; //902
    protected $cover_crop_image_height = 400; //320
...
//        }
        ...
        //checdk for lat and log
       // if($de_serialize['sale_subcatid'] != strtolower('undefined'))?$de_serialize['sale_subcatid']:'';
        ...
        if ($store_vat_number != '') { //validate if vat number is not blank.
            ...
            if((!$valid_vatnumber) && ($vat_mode == self::VAT_MODE)){
                ...
                $res_data = array('code' => '165', 'message' => 'VAT_NUMBER_NOT_VALID', 'data' => array());
                echo json_encode($res_data);
                exit;
            }
        }
        ...
        if((!$valid_iban) && ($vat_mode == self::VAT_MODE)){
            ...
            $res_data = array('code' => '166', 'message' => 'IBAN_NOT_VALID', 'data' => array());
            echo json_encode($res_data);
            exit;
        }
        ...
        if ($store_owner_id == "") {
            $res_data = array('code' => 115, 'message' => 'STORE_OWNER_ID_REQUIRED', 'data' => array());
            return $res_data;
        }
       ...
       /* Get TXN Percentage from bussiness category */

        $cal_txn_charge = 0.06;
        $cal_other_charge = 0.01;

        if($sale_catid > 0){

         $BusinessCategory = $em->getRepository('UserManagerSonataUserBundle:BusinessCategory');

         $getBuscatData = $BusinessCategory->getCategoryName('en',$sale_catid);

         if(isset($getBuscatData[0]['txn_percentage']) && $getBuscatData[0]['txn_percentage'] > 0 ){
            $cal_txn_charge = $getBuscatData[0]['txn_percentage']/100;
            $cal_other_charge =  $cal_txn_charge/6;
           }
        }

        /* End Calculation for TXN Charge */
...
//            $res_data = array('code' => '138', 'message' => 'VAT_NUMBER_ALREADY_EXIT', 'data' => array());
//            echo json_encode($res_data);
//            exit;
//        }
        //get store object
        ...
        $store->setstoreImage('');
        $store->setName($store_name);
        $store->setpaymentStatus('0');
        $store->setcreditCardStatus('0');
        $store->setUpdatedAt($time);
        $store->setshopStatus('0');
        $store->settotalDp('0');
        $store->setbalanceDp('0');
        $store->setFiscalCode($fiscal_code);
        $store->setSaleCountry($sale_country);
        $store->setSaleRegion($sale_region);
        $store->setSaleCity($sale_city);
        ...
        $store->setRepresZip($repres_zip);
        $store->setShopKeyword($shop_keyword);
        $store->setavgRating('0');
        $store->setvoteCount('0');
        $store->setNewContractStatus(1);
        $store->setisSubscribed('0');
        $store->setisPaypalAdded('0');

        $store->settxnPercentage($cal_txn_charge);

        $store->setcardPercentage('0');
        $store->setaffiliationStatus('0');
        $store->setcitizenAffCharge($cal_other_charge);
        $store->setshopAffCharge($cal_other_charge);
        $store->setfriendsFollowerCharge($cal_other_charge);
        $store->setbuyerCharge($cal_other_charge);
        $store->setsixcCharge($cal_other_charge);
        $store->setallCountryCharge($cal_other_charge);
...
//    }
    ...
    public function postGetuserstoresAction(Request $request) {
        ...
        $stores = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->getStoresWithOffers($user_id, $limit_start, $limit_size, $store_type,$bucket_path,$filter_type,$citizen_income,$language_code,$friendsIds);
        ...
        foreach($stores as $store) {
            $store['credit_available'] = isset($user_info_store[$store['id']]) ? $user_info_store[$store['id']] : 0;
            $store_result[] = $store;
        }
        ...
        $stores_count = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->getStoresWithOffersCount($user_id, $store_type);
...
//            $store_vate_number = count($store_vat);
//            if ($store_vate_number > 0) {
//                $res_data = array('code' => 91, 'message' => 'VAT_NUMBER_ALREADY_EXIT', 'data' => array());
//                echo json_encode($res_data);
//                exit;
//            }
//        }
        ...
        if (!in_array($store_allow_access, $forum_access_array)) {
            $resp_data = array('code' => '126', 'message' => 'INVALID_STORE_FORUM_TYPE', 'data' => array());
            return $resp_data;
        }
        //get store owner id
        $store_owner_id = $de_serialize['user_id'];
        ...
        if ($user_check_enable == false) {
            $res_data = array('code' => 90, 'message' => 'ACCOUNT_IS_NOT_ACTIVE', 'data' => $data);
            return $res_data;
        }
        ...
        if ($post_type == 0) {
            ...
            foreach ($_FILES['store_media']['tmp_name'] as $key => $tmp_name) {
                ...
                if ($original_media_name) {
                    ...
                    //   $image_upload->imageUploadService($_FILES['store_media'],$key,$store_id,'store_album',$file_name,$pre_upload_media_dir,$media_original_path,$thumb_dir,$thumb_crop_dir,$s3_post_media_path,$s3_post_media_thumb_path,$store_album_id);
                        $image_upload->albumimageUploadService($_FILES['store_media'],$key,$store_id,'store_album',$file_name,$pre_upload_media_dir,$media_original_path,$thumb_dir,$thumb_crop_dir,$s3_post_media_path,$s3_post_media_thumb_path,$store_album_id);
                    }
                }
            }
            ...
            if (!empty($media_id_arr)) {
                ...
                if(!empty($tagged_friends)){
                    $item_id = isset($media_id_arr[0]) ? $media_id_arr[0] : 0;
                    $_storeMedia = $dm->getRepository('StoreManagerPostBundle:ItemRating')
                            ->findOneBy(array('item_id'=>(string)$item_id,'item_type'=>'store_media'));
                    $existingTaggedFriends = array();
                    if(!$_storeMedia){
                        $_storeMedia = new ItemRating();
                        $_storeMedia->setItemId($item_id);
                        $_storeMedia->setItemType('store_media');
                    }else{
                        $existingTaggedFriends = $_storeMedia->getTaggedFriends();
                    }
                    $_storeMedia->setTaggedFriends($tagged_friends);
                    $dm->persist($_storeMedia);
                    $dm->flush();

                    if(count($existingTaggedFriends)){
                        $fid = array_diff($tagged_friends, $existingTaggedFriends);

                    } else {
                        $fid = $tagged_friends;
                    }
                    $this->_sendTaggedFriendNotifications($fid, $store_owner_id, $store_album_id, $item_id, $store_id);
                }
            ...
            if ($all_publish_media) {
                ...
                foreach ($all_publish_media as $record) {
                    ...
                    $_storeMedia = $dm->getRepository('StoreManagerPostBundle:ItemRating')
                            ->findOneBy(array('item_id'=>(string)$record->getId(),'item_type'=>'store_media'));

                    $tagged_friends_info = array();
                    if($_storeMedia){
                        $tagged_user_ids = $_storeMedia->getTaggedFriends();
    ...
    public function postStoredetailsAction(Request $request) {
        ...
        foreach ($store_images as $store_image) {
            ...
            $img_path[] = array('image_id' => $store_image->getId(),
                'path' => $this->getS3BaseUri() . "/uploads/documents/stores/gallery/" . $store_id . "/original/" . $store_image->getimageName(),
                'thumb_path' => $this->getS3BaseUri() . "/uploads/documents/stores/gallery/" . $store_id . "/thumb/" . $store_image->getimageName(),
                'cover_path' => $this->getS3BaseUri() . "/uploads/documents/stores/gallery/" . $store_id . "/thumb_cover_crop/" . $store_image->getimageName(),
                'is_featured' => $store_image->getIsFeatured(),
      ...
      $ctore_compare_date = new \DateTime('2014-11-14');
       $created_at_date = new \DateTime($store_detail[0]['created_at']);

        if($created_at_date < $ctore_compare_date) {
            $old_shop = 1;
        ...
        $store_profile_images = $em->getRepository('UserManagerSonataUserBundle:BusinessCategory')
                                   ->getCategoryImageFromStoreIds($store_id);
        //prepare store info array
        $store_data = array(
            'id' => $store_id,
            ...
            'is_following' => $store_detail[0]['is_following'],
            'new_contract_status'=>(int)$store_detail[0]['new_contract_status'],
            'is_subscribed' => (int) $store_detail[0]['is_subscribed'],
            'is_paypal_added' => (int) $store_detail[0]['is_paypal_added'],
            'affiliation_status' => (int) $store_detail[0]['affiliation_status'],

            'citizenAffCharge' => (float) $store_detail[0]['citizen_aff_charge'],
            'shopAffCharge' => (float) $store_detail[0]['shop_aff_charge'],
            'friendsFollowerCharge' => (float) $store_detail[0]['friends_follower_charge'],
            'buyerCharge' => (float) $store_detail[0]['buyer_charge'],
            'sixcCharge' => (float) $store_detail[0]['sixc_charge'],
            'allCountryCharge' => (float) $store_detail[0]['all_country_charge'],
        ...
        $store_data['x_cord'] = $x;
        $store_data['y_cord'] = $y;
        $store_data['media_id'] = $current_store_profile_image_id;
        ...
        $stores_revenue = $em
                ->getRepository('StoreManagerStoreBundle:Transactionshop')
                ->getShopsRevenue($store_id);
        //$stores_revenue['tot_fatturato'] = 0;
...
//    }
    ...
    public function postDeletestoresAction(Request $request) {
        ...
        $appalne_data = array('shop_id'=> $store_id);

        //get dispatcher object
        $event = new FilterDataEvent($appalne_data);
        $dispatcher = $this->container->get('event_dispatcher');
        $dispatcher->dispatch('shop.delete', $event);
...
//    }
    ...
    public function postSearchstoresAction(Request $request) {
        ...
        $search_stores = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->searchStoresWithOffer($user_id,$store_business_name, $limit_start, $limit_size,$bucket_path,$language_code,$citizen_income,$friendsIds);
        if (!$search_stores) {
            $res_data = array('code' => 121, 'message' => 'NO_STORE_FOUND', 'data' => $data);
            return $res_data;
        }
        ...
        $search_stores_count = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->countSearchStoresWithOffer($store_business_name);
        ...
        foreach ($search_stores as $search_store) {
            ...
            $store_data = array(
                ...
                'user_info' => $user_object,
                'is_fav' => $search_store['is_fav'],
                'type' => $search_store['type'],
                'price' => $search_store['price']
            );
            $current_store_profile_image_id = $search_store['storeImage'];
            $store_profile_image_path = $search_store['profile_image_original'];
            $store_profile_image_thumb_path = $search_store['profile_image_thumb'];
            if (!empty($current_store_profile_image_id)) {
                ...
                if (isset($store_profile_images[$store_id]['thumb_image']) && $store_profile_images[$store_id]['thumb_image'] != null and $search_store['type']=='') {
                    $store_profile_image_thumb_path = $store_profile_images[$store_id]['thumb_image'];
                    $store_profile_image_path = $store_profile_images[$store_id]['original_image'];
                }
            }
            $store_data['profile_image_original'] = $store_profile_image_path;
            $store_data['profile_image_thumb'] = $store_profile_image_thumb_path;
            $store_data['credit_available'] = isset($user_info_store[$store_id]) ? $user_info_store[$store_id] : 0;
    ...
    */
    ...
    public function postUploadstoreprofileimagesAction(Request $request) {
        ...
        //check for web service only, not for mobile
        ...
        if (!empty($original_media_name)) { //if file name is not exists means file is not present.
            ...
            $tim = uniqid();
            if($device_request_type == 'mobile'){
                     $flName = gettype($original_media_name)=='array' ? preg_replace('/\s+/', '', $original_media_name[0]) : preg_replace('/\s+/', '', $original_media_name);
            }else{
                    $flName = preg_replace('/\s+/', '', $original_media_name);
            }
            $store_media_name = "{$tim}" . $flName;
...
//            $store_media_name = $clean_name->cleanString($store_media_name); //rename the file name, clean the image name.
            ...
            $store_media_type = gettype($store_media_type)=='array' ? $store_media_type[0] : $store_media_type;
            ...
            $storeMedia->setX('');
            $storeMedia->setY('');
            $storeMedia->stroeProfileImageUpload($store_id, $store_media_name, true); //uploading the files.
            $em->persist($storeMedia);
            $em->flush();
           ...
           // $store_profile_cover_image_thumb_path = $this->getS3BaseUri() . $this->store_media_path . $store_id . '/thumb_cover_crop/' . $store_media_name;
            ...
            $data = array(
                ...
                'cover_image_thumb_path' => $store_profile_cover_image_thumb_path,
                'media_id' => $store_media_id
            );
            $res_data = array('code' => '101', 'message' => 'SUCCESS', 'data' => $data);
    ...
    public function postSetstoreprofileimagesAction(Request $request) {
        ...
        $s3_path = "uploads/documents/stores/gallery/" . $store_id . "/original";
        $image_upload = $this->get('amazan_upload_object.service');// s3 service for getting s3 object
        if ($store_album_id > 0) { //inside a album
            ...
            $file_local_path = $media_original_path;
            $image_upload->ImageS3DownloadService($file_local_path, $s3_path, $store_id, $store_media_name, $store_album_id, 'shop');
        } else { //without album
            ...
            $file_local_path = $media_original_path;
            $image_upload->ImageS3DownloadService($file_local_path, $s3_path, $store_id, $store_media_name, $album_id, 'shop');
        }
        ...
        //create the thumb for cover image
        $this->createCoverThumbFromCrop($store_media_name, $media_original_path, $thumb_cover_crop_dir, $store_id,$store_album_id);
       ...
       // $this->cropCoverFromRemote($store_media_name, $media_original_path_covered_croped, $thumb_cover_dir, $store_id, $store_album_id);
        ...
        if ($store_album_id != '') {
          ...
          //  $store_profile_cover_image_thumb_path = $this->getS3BaseUri() . $this->store_media_path . $store_id . '/thumb/coverphoto/' . $store_media_name;
            $store_profile_cover_image_thumb_path = $this->getS3BaseUri() . $this->store_media_path . $store_id . '/thumb/coverphoto/' . $store_album_id .'/'. $store_media_name;
...
//                                                );
     ...
     * @param string $store_id
     */
    public function createCoverThumbFromCrop($filename, $media_original_path, $thumb_crop_dir, $store_id,$store_album_id=0) {
        // $path_to_thumbs_directory = __DIR__ . "/../../../../web/uploads/documents/stores/gallery/" . $store_id . "/thumb_crop/";
       ...
       // imagecopyresampled($nm, $im, 0, 0, 0, 0, $nx, $ny, $ox, $oy);
        ...
        if ($store_album_id != "") {
            $s3imagepath = "uploads/documents/stores/gallery/" . $store_id . '/thumb/coverphoto/' . $store_album_id ;
        ...
        if ($stores) {
            ...
            foreach ($stores as $store_obj) {
                ...
                $data_obj = array(
                    ...
                    'new_contract_status'=>(int)$store_obj['newContractStatus']
                );
  ...
  /**
    ...
    public function postGetmapstoreslistsAction(Request $request) {
      ...
      //echo "postGetmapstoreslistsAction";
        ...
        $count = count($stores);
        $dataResponse = $stores;
        $res_data = array('code' => 101, 'message' => 'SUCCESS', 'data' => $dataResponse, 'count' => $count);
    ...
    public function postSearchshoponmapsAction(Request $request)
    {
       ...
       $shop_array = is_array($shop_array) ? array_unique($shop_array) : $shop_array;
...
//                    ->findOneBy(array('storeId' => $current_store_id, 'role' => 15));
            ...
            $store_data[0]['credit_available'] = isset($user_info_store[$shop_id]) ? $user_info_store[$shop_id] : 0;
    ...
    public function postSearchstoreonfiltersAction(Request $request) {
        ...
        $stores = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->getFilterStoreWithOffer($user_id,$search_text,$search_city,$category,$lat_long,$radius,$sort_type,$limits,$citizen_income,$bucket_path,$store_type,$language_code,$favorite_shop);
...
//        }
        ...
        $stores_count = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->getFilterStoresWithOfferCount($user_id,$search_text,$search_city,$category,$lat_long,$radius,$sort_type,$limits,$citizen_income,$bucket_path,$store_type,$language_code,$favorite_shop);
        ...
        if ($call_type == 'APPLANE') { //from applane
          //call applane service to calculate shop revenue
           $applane_service = $this->container->get('appalne_integration.callapplaneservice');
           $revenue = $applane_service->getShopRevenueFromApplane($store_id);
           ...
           $stores_revenue = $em
                ->getRepository('StoreManagerStoreBundle:Transactionshop')
                ->getShopsRevenue($store_id);
           //check if shop revenue present otherwise set to 0
           $revenue = 0;
           if(count($stores_revenue) > 0) {
            $revenue = $stores_revenue['tot_fatturato'];
            }
        }
    ...
    /*
     * save x and y cordinate for store cover image thumb
     */
    public function postGetstorecovermediacoordinatesAction(Request $request){
     ...
     * @return int|array
     */
    public function postSearchstoreondimensionsAction(Request $request) {
        //initilise the array
        $data = array();
        ...
        //session id will be the current loggedin user id
        $required_parameter = array('longitude','latitute','session_id');
        ...
        $latitute=$de_serialize['latitute'];
        ...
        $stores = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->getFilterStoreondimension($longitude,$latitute,$radius,$limit_start,$limit_size);
        $stores_count = $em
                ->getRepository('StoreManagerStoreBundle:Store')
                ->getcountStoreondimension($longitude,$latitute,$radius);

        $response = array('stores'=>$stores, 'count'=>$stores_count);
        $data =  array('code' => 101, 'message' =>'SUCCESS', 'data' =>$response);
        echo json_encode($data);
        exit();
    }

    private function _sendTaggedFriendNotifications($fid, $user_id, $album_id, $media_id, $store_id){
        if(count($fid))
        {
            //find user object service..
            $user_service = $this->get('user_object.service');
            ...
            $postService->sendUserNotifications($sender['id'], $fid, $msgtype, $msg, $media_id, true, true, $sender_name, 'CITIZEN', array('store_id'=>$store_id, 'album'=>$album_id));
            ...
            foreach($recieverByLanguage as $lng=>$reciever){
                ...
                $mail_text = sprintf($lang_array['TAGGED_IN_STORE_PHOTO_TEXT'], ucwords($sender_name));
                $href = $email_template_service->getPageUrl(array('supportId'=>$user_id, 'parentId'=> $album_id, 'mediaId'=>$media_id, 'albumType'=>'shop'),'single_image_page');
                $bodyData =  $mail_text."<br><br>".$email_template_service->getLinkForMail($href,$locale); //making the link html from service
                $subject = sprintf($lang_array['TAGGED_IN_STORE_PHOTO_SUBJECT'], ucwords($sender_name));
                $mail_body = sprintf($lang_array['TAGGED_IN_STORE_PHOTO_BODY'], ucwords($sender_name));

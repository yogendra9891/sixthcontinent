<?php
namespace Payment\PaymentProcessBundle\Services;
...
class TransactionManagerService
{
    ...
    public function userCards($user_id, $shop_id, $amount, $credit_level) {
        ...
        $coupons = $premimum = $gift_cards = $momosy_cards = $total_citizen_income = $balanced_gift_cards = $open_shop_premimum = 0; //initialize the variables.
        ...
        if ($shop_user_credit) {
            ...
            $balanced_gift_cards   = $shop_user_credit->getBalanceGiftCard();
        ...
        //finding the user discount position.
        $shop_open_discount_position = $this->em->getRepository('StoreManagerStoreBundle:Store')
                                       ->findOneBy(array('id' => $shop_id));
        if ($shop_open_discount_position) {
            $open_shop_premimum             = $shop_open_discount_position->getBalanceDp();
        }
            
        //get gift card dynamically
        $amount_half= $amount/2;
        
        //we assume that at one time a transaction will be initiated once.
        //code start for check if a user already have unapproved transaction.
        $un_approved_transactions = $this->em->getRepository('PaymentPaymentProcessBundle:PaymentProcessCredit')
                                       ->findBy(array('userId' => $user_id, 'shopStatus'=>'PENDING'));
        if (count($un_approved_transactions)) {
            $used_premium = $gift_cards = array();
            foreach ($un_approved_transactions as $transactions) {
                $used_premium[] = $transactions->getUsedPremiumPosition();
                $gift_cards[]   = $transactions->getUsedGiftCard();
            }
            $last_used_premium    = array_sum($used_premium);
            $last_used_gift_cards = array_sum($gift_cards);
            $total_citizen_income = $total_citizen_income-$last_used_gift_cards;
            $premimum             = $premimum-$last_used_premium;
            if ($total_citizen_income < 0) {
        ...
        //code end for check if a user already have unapproved transaction.
        
        //set the cards variables by available amount.
        $this->coupons   = $coupons;
        $this->premimum  = $premimum;
        $this->gift_card = $total_citizen_income;
    ...
    public function getTransactionObject($tid)
    {
        ...
        $tm = $em->getRepository('PaymentPaymentProcessBundle:PaymentProcessCredit')
...
//        }
        if(count($gift_card_packets)>0) {
        foreach($gift_card_packets as $gift_card_arr_rec) {
                    $gift_card_packet_euro[] = "{$this->converToEuro($gift_card_arr_rec)}";
                }
         }
        ...
        $transaction_data_response = array(
            ...
            'is_calculated' => $tm->getIsCalculated()

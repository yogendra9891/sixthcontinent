...
class RestRegistrationController extends BaseController {
    ...
    public function registerAction(Request $request = null) {
        $this->_log('[Entering in RestRegistrationController->registerAction(Request)]');
...
//        }
        //set verification status for the user
        /**
         * verficationstatus=array{'unverified','verified','trail_expired'};
         */
        ...
        if ($type == 1) {
            ...
            $this->_log('Registering TYPE 1 USER[USER_ID=' . $this->_toJSON($register_id) . ']');
            ...
            if ($set_default_citizen == 5) {
                ...
                $res_data = array('code' => '134', 'message' => 'CITIZEN_REGISTERD_ALREADY', 'data' => array());
                echo json_encode($res_data);
                exit;
            }
            ...
            if (!$user) {
                ...
                return array('code' => 93, 'message' => 'USER_DOES_NOT_EXIT', 'data' => $data);
            }
        ...
        if ($type == 3) {
            ...
            $user = $this->container->get('fos_user.user_manager')->findUserBy(array('id' => $register_id));
            if (!$user) {
                $res_data = array('code' => 93, 'message' => 'USER_DOES_NOT_EXIT', 'data' => $data);
                echo json_encode($res_data);
                exit;
            }
    ...
    public function registermultiprofileAction(Request $request) {
        ...
        if ($type == 2) {
            ...
            if ($response == 7) {
                $res_data = array('code' => 92, 'message' => 'BROKER_DOES_NOT_EXIST', 'data' => array());
                echo json_encode($res_data);
                exit;
            }
            if ($response == 5) {

...
class RestGroupAlbumController extends Controller
{
   ...
   public function getS3BaseUri() {
     ...
     */
     public function postUploadgroupmediaalbumsAction(Request $request) {
      ...
      //  $user_id    =  $object_info->session_id = $object_info->user_id ;
        $user_id   = isset($object_info->user_id) ? $object_info->user_id :$object_info->session_id;
               ...
               // media uploading
                    ...
                    if(!empty($tagged_friends)){
                        $group_album_media = $dm->getRepository('UserManagerSonataUserBundle:GroupMedia')
                                            ->findOneBy(array('albumid' => $album_id, 'group_id' => $group_id, 'id'=>$media_array[0]));
                        $existingTaggedFriends = $group_album_media->getTaggedFriends();
                    }
                    //publish all uploaded media and set tagged users
                    $medias = $dm->getRepository('UserManagerSonataUserBundle:GroupMedia')
                                    ->publishClubAlbumImage($media_array,$tagged_friends);
                    ...
                    foreach ($group_album_medias as $album_media) {
                        if(in_array($album_media->getId(),$media_array) && ( is_array($object_info->tagged_friends)) ){
                                ...
                                if(count($fid))
                                {
                                    ...
                                    foreach($recieverByLanguage as $lng=>$reciever){
                                        ...
                                        $href = $email_template_service->getPageUrl(array('supportId'=>$user_id, 'parentId'=> $album_id, 'mediaId'=>$media_id, 'albumType'=>'club'),'single_image_page');

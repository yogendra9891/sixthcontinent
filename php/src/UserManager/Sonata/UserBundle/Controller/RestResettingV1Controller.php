 ...
 */
class RestResettingV1Controller extends BaseController
{
    ...
    public function forgetPasswordAction(Request $request=null)
    {
    	...
    	if($username==""){
    		$res_data = array('code'=>1030,'message'=>'INVALID_USERNAME','data'=>array());
    	...
    	if ($user->isPasswordRequestNonExpired($this->container->getParameter('fos_user.resetting.token_ttl'))) {
    		$res_data = array('code'=>1031,'message'=>'THE_PASSWORD_FOR_THIS_USER_HAS_ALREADY_BEEN_REQUESTED_WITHIN_THE_LAST_24_HOURS','data'=>array());
    		$res_data_serialize = $this->encodeData($res_data);
    		echo $res_data_serialize;
    		exit;
   ...
   private function getAdminId() {
    ...
    public function resetAction($token=null)
    {
       ...
       if (isset($fde_serialize)) {
    	...
    	//check for password
    	if($password == "") {
    		$res_data = array('code'=>1036,'message'=>'INVALID_PASSWORD','data'=>array());
    		$res_data_serialize = $this->encodeData($res_data);
    		echo $res_data_serialize;
    		exit;
    	}
        ...
        //check for token
        if(!$user){
        	$res_data = array('code'=>1037,'message'=>'INVALID_TOKEN','data'=>array());
        	$res_data_serialize = $this->encodeData($res_data);
        	echo $res_data_serialize;
        	exit;
        }
        ...
        if (null === $user) {
        	...
        	$res_data = array('code'=>1038,'message'=>sprintf('The user with confirmation token does not exist for value %s', $token),'data'=>array());
        	$res_data_serialize = $this->encodeData($res_data);
        	echo $res_data_serialize;
        	exit;

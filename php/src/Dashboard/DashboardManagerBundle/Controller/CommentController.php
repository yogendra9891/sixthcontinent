...
class CommentController extends Controller implements ApplaneConstentInterface{
    ...
    public function postDashboarddeletecommentsAction(Request $request) {
        ...
        $post_id = $comment_res->getPostId(); //find post id of comment.

        $post_res = $dm->getRepository('DashboardManagerBundle:DashboardPost')
                    ->find($post_id); //find post object.
        if (!$post_res) {
            return array('code' => 100, 'message' => 'RECORD_DOES_NOT_EXISTS', 'data' => $data);
        }
        $post_owner_id = $post_res->getUserId(); //get post owner id
        
        //for store ACL     
        $do_action = 0;
        $group_mask = $this->userCommentRole($object_info->comment_id, $object_info->user_id);

        //check for the current user is post owner.
        if ($post_owner_id == $object_info->user_id) {
            $do_action = 1;
        }
        //check for Access Permission
        //only owner and admin can edit the group
...
//        }
        ...
        //finding the sender and receiver users for showing friendship type..
        $users_array = $user_relations = array();
        if (count($posts_data)) {
            $feed_service = $this->container->get('dashboard_feed.service'); //Dashboard\DashboardManagerBundle\Services\DashboardFeedsService
            $users_array  = $feed_service->getFeedUsers($posts_data);
            $user_relations = $feed_service->getUserFriendShipRelations($users_array);
        }
        //finding the sender and receiver users for showing friendship type.
        if (count($posts_data) > 0) {
            if ($current_user_id == $friend_id) { // if a user seeing his dashboard.
        ...
        if (count($post_ids)) {
            ...
            foreach ($posts_data as $post_data) {
                ...
                $friendship_type = $personal_status = $proffessional_status = 0;
                //cheking for personal friendship
                //commented for removing the multiple queries in loop
              /*  $personal_friend_check = $em->getRepository('UserManagerSonataUserBundle:UserConnection')
                ...
                //code for checking the friendship type
                if (isset($user_relations[self::PERSONAL][$sender_id])) { //personal check
                    if (in_array($receiver_id, $user_relations[self::PERSONAL][$sender_id])) {
                        $personal_status = 1;
                        $friendship_type = 1;
                    }
                }
                if (isset($user_relations[self::PERSONAL][$receiver_id])) { //personal check
                    if (in_array($sender_id, $user_relations[self::PERSONAL][$receiver_id])) {
                        $personal_status = 1;
                        $friendship_type = 1;
                    }
                }
                if (isset($user_relations[self::PROFESSIONAL][$sender_id])) { //professional check
                    if (in_array($receiver_id, $user_relations[self::PROFESSIONAL][$sender_id])) {
                        $proffessional_status = 1;
                        $friendship_type = 2;
                    }
                }
                if (isset($user_relations[self::PROFESSIONAL][$receiver_id])) { //professional check
                    if (in_array($sender_id, $user_relations[self::PROFESSIONAL][$receiver_id])) {
                        $proffessional_status = 1;
                        $friendship_type = 2;
                    }
                }
                if ($personal_status && $proffessional_status) {
                    $friendship_type = 3;
                }
                //code for checking the friendship type
...
//        }
    ...
    public function getDashboardPostDetails($post_id){
        ...
        if (count($post_ids)) {
            ...
            $users_array = array_unique(array_merge(array($post_sender_user_id), $comment_user_ids, $post_tagged_user_unique_ids));
    ...
    public function getShopPostDetails($post_id){
        ...
        if (is_array($post->getTaggedFriends())) {
            $post_tagged_user_ids = $post->getTaggedFriends();
        } else {
    ...
    public function getClubPostDetails($post_id)
    {
       ...
       $comment_ids = array();
        ...
        //getting the tagged user ids.
        if (is_array($post_data->getTaggedFriends())) {
            $post_tagged_user_ids = $post_data->getTaggedFriends();
        } else {
            $tagged_friend = $post_data->getTaggedFriends();
            if (trim($tagged_friend)) {
                $post_tagged_user_ids = explode(',', $tagged_friend);

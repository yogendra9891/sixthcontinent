...
class CRUDController extends Controller
{
    ...
    public function batchAction()
    {
        ...
        //code start for delete batch action.
        //check for delete batch action for district,circle,position 
        if (($finalAction == 'batchActionDelete') && count($idx) > 0) {
            //get entity manager object
            $em = $this->getDoctrine()->getEntityManager();
            foreach ($idx as $id){
                $deleted_user = new UserDeletedAssign(); //get entity class object
                $deleted_user->setUserId($id);
                $deleted_user->setAssignId(0); //set the default value for assign id 0
                $em->persist($deleted_user);
                $em->flush();
            }
        }
        //code edit end for delete batch action.

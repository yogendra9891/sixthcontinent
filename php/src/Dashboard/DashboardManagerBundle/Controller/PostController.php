...
class PostController extends Controller {
    ...
    protected $allowed_object_type = array('club','shop', 'offer', 'social_project', 'external', 'voucher');
...
//                        }
    ...
    public function getEditPostObject($object_data) {
        ...
        $post->setTaggedFriends($object_data->tagged_friends);
        $post->setprivacySetting($object_data->privacy_setting);
        if(isset($object_data->content_share)){
        $post->setContentShare($object_data->content_share);
        }
        //$post->setLinkType($object_data->link_type);
        //$post->setCreatedDate($time);
...
//    }
    ...
    /**
     * Updating Dashboard Post ACL
     * @param object request
     * @return json string
     */
    public function postUpdatedashboardpostaclsAction(Request $request) {
        //Code start for getting the request
        $freq_obj = $request->get('reqObj');
        $fde_serialize = $this->decodeObjectAction($freq_obj);
        ...
        //check required params
        $required_params = array('user_id', 'post_id', 'privacy_setting');
        $this->checkRequiredParams($de_serialize, $required_params);

        //validating params
        $required_fields = array('user_id', 'privacy_setting', 'post_id');
        ...
        $privacy_setting = $de_serialize['privacy_setting'];

        $valid_privacy = array(1, 2, 3);
        if(!in_array($privacy_setting, $valid_privacy)){
            $res_data = array('code' => '131', 'message' => 'INVALID_PRIVACY_SETTING', 'data' => array());
            echo json_encode($res_data);
            exit;
        }
        ...
        if ($de_serialize['user_id'] != $creater_id) {
            $res_data = array('code' => 303, 'message' => 'ACTION_NOT_PERMITED', 'data' => array());
            echo json_encode($res_data);
            exit;
        }

        $post->setprivacySetting($privacy_setting);
    ...
    public function postShareitemsAction(Request $request)
    {
        ...
        $data = array();
        $required_parameter = array('user_id', 'to_id','post_type','content_share');
    ...
    public function sharePostObject($object_data, $sender_user) {
        $this->__createLog('[Entering in Dashboard\DashboardManagerBundle\Controller\PostController->sharePostObject(Request)]');
        //code for responding the current post data..
        $post_data = array();
        $dm = $this->get('doctrine.odm.mongodb.document_manager'); //getting doctrine mongo odm object.
        $object_data->object_type = Utility::getLowerCaseString($object_data->object_type);
        $object_data->share_type = Utility::getLowerCaseString($object_data->share_type);
        $post_id = $object_data->post_id;
        $time = new \DateTime('now');
        $content_share_data = array();
        $content_share = $object_data->content_share;
        //prepare the data for the contant share if te data is not present intilizing it to the default value
        $content_share_data['url'] = isset($content_share['url']) ? $content_share['url'] : '';
        $content_share_data['pageUrl'] = isset($content_share['pageUrl']) ? $content_share['pageUrl'] : '';
        $content_share_data['canonicalUrl'] = isset($content_share['canonicalUrl']) ? $content_share['canonicalUrl'] : '';
        ...
        $this->__createLog('[Exiting from Dashboard\DashboardManagerBundle\Controller\PostController->sharePostObject] With response'.Utility::encodeData($post_data));
        //code end for responding the current post data.
        return $post_data;
    }
    /**

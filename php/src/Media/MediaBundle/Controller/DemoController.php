...
class DemoController extends Controller {
    ...
    protected $cover_image_width = 910; //902
    protected $cover_image_height = 400; //320
    protected $club_cover_image_width = 910; // 902
    protected $club_cover_image_height = 400; //320
           ...
           if($post_type == 0){
            ...
            foreach ($_FILES['user_media']['tmp_name'] as $key => $tmp_name) {
                ...
                //    $image_upload->imageUploadService($_FILES['user_media'],$key,$user_id,'user_album',$user_media_name,$pre_upload_media_dir,$media_original_path,$thumb_dir,$thumb_crop_dir,$s3_post_media_path,$s3_post_media_thumb_path,$album_id);
                    $image_upload->albumimageUploadService($_FILES['user_media'],$key,$user_id,'user_album',$user_media_name,$pre_upload_media_dir,$media_original_path,$thumb_dir,$thumb_crop_dir,$s3_post_media_path,$s3_post_media_thumb_path,$album_id);
...
//                            }
                            ...
                            $postService->sendUserNotifications($sender['id'], $fid, $msgtype, $msg, $media_id, true, true, $sender_name, 'CITIZEN', array('owner'=>$user_id, 'album'=>$album_id));
                            ...
                            foreach($recieverByLanguage as $lng=>$reciever){
                                ...
                                $href = $email_template_service->getPageUrl(array('supportId'=>$user_id, 'parentId'=> $album_id, 'mediaId'=>$media_id, 'albumType'=>'user'),'single_image_page');
...
//                    ->findBy(array('albumid' => $album_id, 'userid' => $user_id, 'enabled'=>1));
    ...
    public function postViewuseralbumsAction(Request $request) {
        ...
        if ($albums) {
           ...
           // echo "<pre>"; print_r($album_medias); exit;
            ...
            $resp_data = array('code' => '100', 'message' => 'NO_RECORDS_EXITS', 'data' => $data);
            echo json_encode($resp_data);
            exit();
        }
    }
    ...
    public function postUploaduserprofileimagesAction(Request $request) {
        ...
        if (!empty($original_media_name)) { //if file name is not exists means file is not present.
            ...
            if ($media_id) {
                ...
                $user_object_profile_img_org = $user_object['profile_image'];
                // call applane service  
                $appalne_data = array('profile_img' =>$user_object_profile_img, 'profile_img_org' => $user_object_profile_img_org, 'user_id'=> $user_id);
    ...
    public function postSetclubprofileimagesAction(Request $request) {
        ...
        $s3_path = 'uploads/groups/original/';
        $album_id = '';
        ...
        if ($media_info){
            ...
            if($album_id){
                ...
                $image_upload->ImageS3DownloadService($file_local_path, $s3_path, $group_id, $group_media_name, $album_id);
            } else {
           ...
           //set cover image
           $this->createClubCoverImageThumbnail($group_media_name, $media_original_path, $thumb_dir, $group_id,$album_id);
    ...
    public function postSetusercoverimagesAction(Request $request) {
        ...
        if (!empty($original_media_name)) { //if file name is not exists means file is not present.
            ...
            */
            ...
            $user_media->setContenttype($user_media_type);
            $user_media->setAlbumid('');
            $user_media->setX(''); // save x co-ordinate null
            $user_media->setY(''); // save y co-ordinate null
            $user_media->profileImageUpload($user_id, $user_media_name);
            $dm->persist($user_media);
            $dm->flush();
   ...
   /**
     ...
     * @param type $group_id
     */
    public function createClubCoverImageThumbnail($filename, $media_original_path, $thumb_dir, $group_id,$group_album_id='')
    {
    ...
    /*
     * save x and y cordinate for cover image thumb
     */
    public function postGetmediacoordinatesAction(Request $request){
        $data = array();
        //Code start for getting the request
        $freq_obj = $request->get('reqObj');
        $fde_serialize = $this->decodeDataAction($freq_obj);

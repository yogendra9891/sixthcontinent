...
class AlbumV1Controller extends Controller
{
   ...
   /**
    ...
    public function postEditalbumsAction(Request $request) {
        ...
        if(!in_array($album_type,$type_array)) {
            return array('code'=>1009, 'message'=>'ALBUM_TYPE_IS_INVALID','data'=>$data); 
        }
    ...
    public function editUserAlbum($object_info) {
        ...
        if (!in_array($album_privacy_setting, $privacy_setting_constant_result)) {
            return array('code' => 1010, 'message' => 'YOU_HAVE_PASSED_WRONG_PRIVACY_SETTING', 'data' => $data);
        }
    ...
    public function editStoreAlbum($object_info) {
        ...
        $store_album->setStoreAlbumName($album_desc);
        $store_album->setStoreAlbumDesc($album_name);

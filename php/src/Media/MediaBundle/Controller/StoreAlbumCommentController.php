...
use Media\MediaBundle\Document\StoreAlbumComment;
use Media\MediaBundle\Document\StoreAlbumCommentMedia;
use Notification\NotificationBundle\NManagerNotificationBundle;
use Media\MediaBundle\Controller\AlbumCommentController;
use StoreManager\StoreBundle\Controller\RestStoreController;

class StoreAlbumCommentController extends Controller
{
    protected $storealbum_comment_thumb_image_width = 654;
    protected $storealbum_comment_thumb_image_height = 360;
    protected $storealbum_comment_original_resize_image_width = 910;
    protected $storealbum_comment_original_resize_image_height = 910;
    protected $store_album_comment_msg ='COMMENT';
    protected $store_album_comment_type ='STORE_ALBUM_COMMENT';
    protected $store_album_comment_on_commented_type ='STORE_ALBUM_COMMENT_ON_COMMENTED';
    ...
    /*
     * create comment for store album
     * @param type $object_info
     */

    public function createStoreAlbumComment($object_info){
        $data = array();
        ...
        $album_owner_id = $em->getRepository('StoreManagerStoreBundle:StoreAlbum')->getStoreOwnerId($album_id);//get album owner id
        $store_id = $store_album->getStoreId();
        $albumTitle = $store_album->getStoreAlbumName();
        ...
        if ($comment_type == 0) {
            if($object_info->comment_id == 0){
                $store_album_comment = new StoreAlbumComment();
                $store_album_comment->setAlbumId($album_id);
                $store_album_comment->setCommentAuthor($user_id);
                $store_album_comment->setCommentText($comment_body);
                $store_album_comment->setCommentCreatedAt($time);
                $store_album_comment->setCommentUpdatedAt($time);
                $store_album_comment->setTagging($tagging);
                $store_album_comment->setStatus(0); // 0=>disabled, 1=>enabled
                $store_album_comment->setAlbumType($album_type); // album type = user, store, store

                $dm->persist($store_album_comment); //storing the comment data.
                $dm->flush();
                $comment_id = $store_album_comment->getId(); //getting the last inserted id of comments.
                 //update ACL for a user
                $album_comment->updateAclAction($sender_user, $store_album_comment);
            } else {
            ...
            if (isset($_FILES['commentfile'])) {
                ...
                foreach ($_FILES['commentfile']['tmp_name'] as $key => $tmp_name) {
                    ...
                    $storealbum_comment_thumb_image_width  = $this->storealbum_comment_thumb_image_width;
                    $storealbum_comment_thumb_image_height = $this->storealbum_comment_thumb_image_height;
                    ...
                    if (!empty($original_file_name)) { //if file name is not exists means file is not present.
                        ...
                        $image_type         = $image_type_service->CheckImageType($orignal_mediaWidth,$original_mediaHeight,$storealbum_comment_thumb_image_width,$storealbum_comment_thumb_image_height);


                        $dm = $container->get('doctrine.odm.mongodb.document_manager'); //getting doctrine mongo odm object.
                        $storealbum_comment_media = new StoreAlbumCommentMedia();
                        if (!$key) //consider first image the featured image.
                            $storealbum_comment_media->setIsFeatured(1);
                        else
                            $storealbum_comment_media->setIsFeatured(0);
                        $storealbum_comment_media->setCommentId($comment_id);
                        $storealbum_comment_media->setMediaName($file_name);
                        $storealbum_comment_media->setMediaType($actual_media_type);
                        $storealbum_comment_media->setCreatedAt($time);
                        $storealbum_comment_media->setUpdatedAt($time);
                        $storealbum_comment_media->setPath('');
                        $storealbum_comment_media->setIsActive(0);
                        $storealbum_comment_media->setImageType($image_type);
                        $dm->persist($storealbum_comment_media);
                        $dm->flush();

                        //get the storealbum comment media id
                        $storealbum_comment_media_id = $storealbum_comment_media->getId();

                        //update ACL for a store
                        //  $album_comment->updateAclAction($sender_user, $store_album_comment);
                        $pre_upload_media_dir = __DIR__ . "/../../../../web" . $container->getParameter('storealbum_comment_media_path'). $comment_id . '/';
                        $media_original_path = __DIR__ . "/../../../../web" . $container->getParameter('storealbum_comment_media_path') . $comment_id . '/';
                        $thumb_dir = __DIR__ . "/../../../../web" . $container->getParameter('storealbum_comment_media_path_thumb') . $comment_id . '/';
                        $thumb_crop_dir = __DIR__ . "/../../../../web" . $container->getParameter('storealbum_comment_media_path_thumb_crop') . $comment_id . "/";
                        $s3_post_media_path = $container->getParameter('s3_storealbum_comment_media_path'). $comment_id ;
                        $s3_post_media_thumb_path = $container->getParameter('s3_storealbum_comment_media_thumb_path'). $comment_id;
                        $image_upload->imageUploadService($_FILES['commentfile'],$key,$comment_id,'storealbum_comment',$file_name,$pre_upload_media_dir,$media_original_path,$thumb_dir,$thumb_crop_dir,$s3_post_media_path,$s3_post_media_thumb_path);
            ...
            $comment_media_data = $dm->getRepository('MediaMediaBundle:StoreAlbumCommentMedia')
                                     ->find($storealbum_comment_media_id);
            ...
            if ($comment_media_data) {
                ...
                $comment_media_link  = $this->getS3BaseUri() . $container->getParameter('storealbum_comment_media_path') . $comment_id . '/' . $comment_media_name;
                $comment_media_thumb = $this->getS3BaseUri() . $container->getParameter('storealbum_comment_media_path_thumb') . $comment_id . '/' . $comment_media_name;
            }
            //sending the current media and post data.
            $data = array(
                'id' => $comment_id,
                'media_id' => $storealbum_comment_media_id,
            ...
            $storeAlbumCommentId = $object_info->comment_id;
            $media_id = $object_info->media_id;
            //calling rating notification service
            $notification_obj = $container->get('post_detail.service');

            if (!empty($media_id)) {
                $media_update_status = $dm->getRepository('MediaMediaBundle:StoreAlbumCommentMedia')
                                          ->publishStoreAlbumCommentMediaImage($media_id);
            }
            if ($storeAlbumCommentId) {
                ...
                $this->sendCommentNotifications($user_id, $album_owner_id, $store_id, $object_info->comment_id, $album_id, $albumTitle, $tagging, $store_name);
                ...
                $this->sendCommentNotifications($user_id, $album_owner_id, $store_id, $comment_data['id'], $album_id, $albumTitle, $tagging, $store_name);
                $final_array = array('code' => 101, 'message' => 'SUCCESS', 'data' => $comment_data);
                echo json_encode($final_array);
                exit;
            }
...
//                }
        ...
        $data = array(
            'id' => $comment_id,
            'store_album_id' => $object_info->album_id,
...
//                }
    ...
    /**
     * Update comment of a store album.........
     * @param request object
     * @return json string
     */
    public function editStoreAlbumComment($object_info) {
        ...
        if ($do_action) {
            ...
            if ($comment_id) {
               // $dm_obj = $this->get('doctrine.odm.mongodb.document_manager'); //getting doctrine mongo odm object.
                $comment_obj = $dm->getRepository('MediaMediaBundle:StoreAlbumComment')
                                      ->find($comment_id);
            }
            ...
            if ($comment_obj) {
               // $dm_obj_media = $this->get('doctrine.odm.mongodb.document_manager'); //getting doctrine mongo odm object.
                $comment_obj_media = $dm->getRepository('MediaMediaBundle:StoreAlbumCommentMedia')
                ...
                if ($album_id) {
                   // $dm_post_object = $this->get('doctrine.odm.mongodb.document_manager'); //getting doctrine mongo odm object.
                    $post_obj_res = $dm->getRepository('MediaMediaBundle:StoreAlbumComment')
                                                   ->find($album_id);
                }
...
//                        }
            ...
            $store_album_url     = $this->getContainer()->getParameter('store_media_url'); //store album url
            $link_url = $angular_app_hostname . $store_album_url . '/'. $album_id.'/'.$albumTitle.'/'. $store_id;

            if(!empty($newTagging)){
                $postService->commentTaggingNotifications($tagging, $album_owner_id, $album_id, $link_url, 'STORE_ALBUM', true, array('album_title'=>$albumTitle, 'store_id'=>$store_id), false, array(), array($store_name));
            }
    ...
    * @return json string
    */
     public function listStoreAlbumComment($object_info) {
        ...
        $store_album_data =$em->getRepository('StoreManagerStoreBundle:Storealbum')->find($album_id);
...
//        $comment_count = $dm->getRepository('MediaMediaBundle:StoreAlbumComment')
...
//                           ->findBy(array('album_id' => $album_id, 'status' => 1), array('comment_created_at' => '1'), $limit, $offset);
        $_comments = $dm->getRepository('MediaMediaBundle:StoreAlbumComment')
        ...
        if (count($comment_res)) {
            ...
            $comments_media = $dm->getRepository('MediaMediaBundle:StoreAlbumCommentMedia')
                                 ->getStoreAlbumCommentMedia($comment_ids);
            ...
            foreach ($comment_res as $comment) {
                ...
                foreach ($comments_media as $media) {
                    if ($media->getCommentId() == $comment_id) {
                        ...
                        $storealbum_comment_id = $media->getCommentId();
                        ...
                        *
                        */
                            $media_link  = $this->getS3BaseUri() . $container->getParameter('storealbum_comment_media_path') . $storealbum_comment_id . '/' . $media_name;
                            $media_thumb = $this->getS3BaseUri() . $container->getParameter('storealbum_comment_media_path_thumb') . $storealbum_comment_id . '/' . $media_name;
                       //  }
                        $comment_media_data[] = array('id' => $media_id,
                            'comment_id' => $storealbum_comment_id,
     ...
     * @return json string
     */
    public function deleteStoreAlbumComment($object_info){
        $data = array();
        $userManager = $this->getUserManager();
        $sender_user = $userManager->findUserBy(array('id' => $object_info->user_id));
        ...
        $storeOwner = $em->getRepository('StoreManagerStoreBundle:Storealbum')
                          ->getStoreOwnerId($albumId);

        if($storeOwner and $storeOwner==$object_info->user_id){
            $do_action = 1;
        }
        //only owner and admin can edit the group
        $allow_group = array('15', '7');
        ...
        if ($do_action) {
            ...
            $comment_media = $dm->getRepository('MediaMediaBundle:StoreAlbumCommentMedia')
                                ->removeStoreAlbumCommentsMedia($object_info->comment_id);
            if ($comment_media) {
    ...
    /**
     * Delete media for comment on store album
     * @param request object
     * @return json string
     */
    public function deleteStoreAlbumCommentMedia($object_info) {
    ...
    public function sendCommentNotifications($from, $owner, $store_id, $comment_id, $album_id, $albumTitle, $tagging, $store_name){
        ...
        $message = $this->store_album_comment_msg;
        $ownerMessageType = $this->store_album_comment_type;
        $commentAuthorMessageType = $this->store_album_comment_on_commented_type;

        $commentedAuthors = $dm->getRepository('MediaMediaBundle:StoreAlbumComment')->getCommentedUserIds($album_id);
        ...
        if($from!=$owner){
            $postService->sendUserNotifications($from, $owner, $ownerMessageType, $message, $album_id, true, true, array($senderName, $store_name), 'CITIZEN', array('album_title'=>$albumTitle, 'store_id'=>$store_id), 'U', array('comment_id'=>$comment_id, 'store_id'=>$store_id));
            ...
            $subject = sprintf($lang_array['STORE_ALBUM_COMMENT_SUBJECT'],$senderName, $store_name);
            $mail_link = sprintf($lang_array['STORE_ALBUM_COMMENT_LINK'],$senderName, $store_name);
            $bodyData = $mail_link.'<br><br>'.sprintf($lang_array['STORE_ALBUM_COMMENT_CLICK_HERE'],$href);
            $bodyTitle = sprintf($lang_array['STORE_ALBUM_COMMENT_BODY'],$senderName, $store_name);

            // HOTFIX NO NOTIFY MAIL
            //$postService->sendMail(array($ownerInfo), $bodyData, $bodyTitle, $subject, $sender['profile_image_thumb'], 'STORE_ALBUM_COMMENT_NOTIFICATION');
        }
        ...
        if(!empty($uniqueAuthors)){
            ...
            foreach($recieverByLanguage as $lng=>$receivers){
                ...
                $href = "<a href= '$link_url'>{$lang_array['CLICK_HERE']}</a>";
                $subject = sprintf($lang_array['STORE_ALBUM_ON_COMMENTED_SUBJECT'],$senderName);
                $mail_link = sprintf($lang_array['STORE_ALBUM_ON_COMMENTED_LINK'],$senderName);
                $bodyData = $mail_link.'<br><br>'.sprintf($lang_array['STORE_ALBUM_ON_COMMENTED_CLICK_HERE'],$href);
                $bodyTitle = sprintf($lang_array['STORE_ALBUM_ON_COMMENTED_BODY'],$senderName);

                // HOTFIX NO NOTIFY MAIL
                //$postService->sendMail($receivers, $bodyData, $bodyTitle, $subject, $sender['profile_image_thumb'], 'STORE_ALBUM_COMMENT_NOTIFICATION');
            }
            $postService->sendUserNotifications($from, $uniqueAuthors, $commentAuthorMessageType, $message, $album_id, true, true, $senderName, 'CITIZEN', array('album_title'=>$albumTitle, 'store_id'=>$store_id), 'U', array('comment_id'=>$comment_id, 'store_id'=>$store_id));
        }

        if(!empty($tagging)){
            $postService->commentTaggingNotifications($tagging, $from, $album_id, $link_url, 'STORE_ALBUM', true, array('album_title'=>$albumTitle, 'store_id'=>$store_id), false, array(), array($store_name));
        }
        return true;
    }

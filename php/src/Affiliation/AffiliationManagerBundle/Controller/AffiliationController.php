...
use Affiliation\AffiliationManagerBundle\Utils\MessageFactory as Msg;
...
class AffiliationController extends Controller {
    ...
    protected $valid_affiliation_type = array(1, 3);
    ...
    public function postSendaffiliationlinksAction(Request $request) {
        ...
        if ($chk_error) {
            $resp_data = new Resp(300, 'YOU_HAVE_MISSED_A_PARAMETER ' . $this->miss_param, $data);
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postSendaffiliationlinksAction] with response: ' . (string) $resp_data);
        ...
        $to_emails_inv = array_unique(array_map('trim', $to_emails));
        //get User
        $userManager = $this->getUserManager();
        $sender_user = $userManager->findUserBy(array('id' => $user_id));
        $from_email_id = $current_language = '';
        if (!$sender_user) {
            $resp_data = new Resp(100, 'USER_ID_IS_INVALID', $data);
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postSendaffiliationlinksAction] with response: ' . (string) $resp_data);
            Utility::createResponse($resp_data);
        ...
        if (!in_array($affiliation_type, $affiliation_type_arr)) {
            $resp_data = new Resp(100, 'AFFILIATION_TYPE_IS_INVALID', $data);
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postSendaffiliationlinksAction] with response: ' . (string) $resp_data);
            Utility::createResponse($resp_data);
        ...
        if ($affiliation_type == 1) {
            ...
            $affiliation_service->addInvitationSend($user_id, $to_emails_inv, $affiliation_type);
            $postService->sendShopInvitation($to_emails, $user_info, $url_to_send, $locale);

            $resp_data = new Resp(101, 'SUCCESS', $data_email);
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postSendaffiliationlinksAction] with response: ' . (string) $resp_data);
            Utility::createResponse($resp_data);
            //return array('code' => 101, 'message' => 'SUCCESS', 'data' => $data_email);
        }
    ...
    public function linkedcitizenAction(Request $request) {
        ...
        $user_ids = array_keys($pagination_array);
        //get user service
        $user_service = $this->get('user_object.service');
        $user_info = $user_service->MultipleUserObjectService($user_ids);

        foreach ($pagination_array as $key => $value) {
            if (array_key_exists($key, $user_info)) {
                $affiliation_count = $value;
                $data[] = array('user_info' => $user_info[$key], 'affiliation_count' => $value);
            }
        }
...
//                } else {
    ...
    /**
     * function for getting the list of all the invited users for a user
     * @param \Symfony\Component\HttpFoundation\Request $request
     */
    public function postUserinvitationlistsAction(Request $request) {
        $affiliation_service = $this->container->get('affiliation_affiliation_manager.user');
        $affiliation_service->__createLog('Enter In class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postUserinvitationlistsAction]');
        $data = array();
        $data_info = array();
        $required_parameter = array('user_id', 'affiliation_type');
        ...
        if ($response !== true) {
            ...
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postUserinvitationlistsAction] with response: ' . (string) $resp_data);
            Utility::createResponse($resp_data);
        }
        ...
        $limit = isset($de_serialize['limit_size']) ? (int) $de_serialize['limit_size'] : false;
        $offset = isset($de_serialize['limit_start']) ? (int) $de_serialize['limit_start'] : false;
        $affiliation_type = (int) $de_serialize['affiliation_type'];
        $affiliation_type_arr = $this->valid_affiliation_type;

        if (!in_array($affiliation_type, $affiliation_type_arr)) {
            $resp_data = new Resp(Msg::getMessage(1117)->getCode(), Msg::getMessage(1117)->getMessage(), $data); //SUCCESS
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postUserinvitationlistsAction] with response: ' . (string) $resp_data);
            Utility::createResponse($resp_data);
        }
        $dm = $this->container->get('doctrine.odm.mongodb.document_manager');
        $results = $dm->getRepository('AffiliationAffiliationManagerBundle:InvitationSend')
                ->getUsersInvitation($user_id, $limit, $offset, $affiliation_type);

        $results_count = $dm->getRepository('AffiliationAffiliationManagerBundle:InvitationSend')
                ->getUsersInvitationCount($user_id, $affiliation_type);

        $users_email = array();
        $users_id = array();
        foreach ($results as $result) {
            $users_email[] = $result->getEmail();
        }

        $affiliation_service->__createLog('In class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postUserinvitationlistsAction] and emails are:' . json_encode($users_email));
//        $affiliation_service = $this->container->get('affiliation_affiliation_manager.user');
//        $user_email_ids = $affiliation_service->getRegisteredEmailsAndIds($users_email);
//        foreach($user_email_ids as $user_email_id) {
//            $users_id[] = $user_email_id['id'];
//        }

        $user_service = $this->container->get('user_object.service');
        $users_data = $user_service->MultipleUserObjectServiceFromEmail($users_email);
        $affiliation_service->__createLog('In class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [postUserinvitationlistsAction] already registerd users are:' . json_encode(array_keys($users_email)));
        foreach ($results as $result) {
            $info['email'] = $result->getEmail();
            $info['status'] = $result->getStatus();
            $info['count'] = $result->getCount();
            $info['last_send'] = $result->getUpdatedAt();
            $info['user_info'] = isset($users_data[$result->getEmail()]) ? $users_data[$result->getEmail()] : array();
            $data_info[] = $info; //mail sent
        }
        $data['invite_users'] = $data_info;
    ...
    public function onetimeaffiliationsaveAction(Request $request) {
        set_time_limit(0);
        ini_set('memory_limit', '512M');
        $data_info = array();
        $affiliation_service = $this->container->get('affiliation_affiliation_manager.user');
        $required_parameter = array('is_limit');
        $store_utility = $this->container->get('store_manager_store.storeUtility');
        $response = $store_utility->checkRequest($request, $required_parameter); //check for request object
        if ($response !== true) {
            $resp_data = new Resp($response['code'], $response['message'], $response['data']);
            $affiliation_service->__createLog('Exiting from class [Affiliation\AffiliationManagerBundle\Controller\AffliationController] function [onetimeaffiliationsaveAction] with response: ' . (string) $resp_data);
            Utility::createResponse($resp_data);
        }

        $de_serialize = $store_utility->getDeSerializeDataFromRequest($request);
        $limit_start = isset($de_serialize['limit_start']) ? : 0;
        $limit_size = isset($de_serialize['limit_size']) ? : 500;
        $is_limit = isset($de_serialize['is_limit']) ? $de_serialize['is_limit'] : 1;

        $em = $this->getDoctrine()->getManager();
        $results = $em->getRepository('AffiliationAffiliationManagerBundle:AffiliationCitizen')
                ->getAlreadyRegisteredAffiliatedUsers($limit_start, $limit_size, $is_limit);
        $affiliation_service->saveAffiliatedUsers($results, 1);
        echo "Done";
        die;
    }

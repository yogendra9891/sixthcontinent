<?php
namespace Utility\CurlBundle\Services;
...
class CurlRequestService {
    ...
    private $options = array(
        ...
        'CURLOPT_USERAGENT'=>'SixthContinent Crawler',
    ...
    /**
     * Set param to send on request url using POST/GET
     * @param array $params
     * @return \CurlRequest
     */
    ...
    /**
     * Get param that will use to send on request url using POST/GET
     * @return type
     */
    ...
    private function updateOptions(){
        ...
        if($this->requestType=='POST'){
            ...
            if(!empty($this->params)){
                $contentType = isset($this->headers['Content-Type']) ? $this->getHeader('Content-Type') : (isset($this->headers['content-type']) ? $this->getHeader('content-type') : '');
                if(preg_match('/(x\-www\-form\-urlencoded)$/',$contentType)){
                    $this->options['CURLOPT_POSTFIELDS'] = $this->getParams(true);
                }else{
                    $this->options['CURLOPT_POSTFIELDS'] = $this->getParams();
                }
            }
        }
    ...
    private function updateHeaders(){
        if(!empty($this->data) and $this->requestType=='POST'){
     ...
     * @param array $headers
     */
    public function send($requestType='', $url='', $params=array(), $data='', $headers=array()){
        ...
        foreach($this->getOptions() as $option=>$value){
            switch ($option){
                ...
                case 'CURLOPT_SSL_VERIFYPEER':
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, $value);
                    break;
            }
        }
    }
    ...
    /**
     *  function for getting the final url that will be hit in curl request
     * @return type
     */
    public function getFinalUrlHit() {
